...
4. 結果一覧の「Markdownを保存」または「すべてZIPで保存」を1回押すと、サイドバーの保存先へ直接保存します。確認ダイアログは出ません。既定の保存先は `%LOCALAPPDATA%\PDF2Markdown\output` で、画面から変更できます。変換設定と保存先は次回起動時にも復元します。同名の既存ファイルは上書きせず、連番を付けます。成功時は保存済みの絶対パスを表示します。結果を開くとMarkdownソースを確認できます。ZIPの `manifest.json` に使用設定・処理時間・状態・エラーを収録します。
5. 「保存された変換履歴・復元」から過去の結果を再変換せずに復元できます。完了を確認できない履歴はその旨を表示します。履歴は本文を含み、このPCに保存されます。「結果をクリア」は画面だけを消し、「選択した履歴を削除」は履歴を削除します。元PDFや別途保存したMarkdown・ZIPは削除しません。

以前のプロジェクト内 `output` はそのまま残しています。引き続き使う場合は保存先欄にその絶対パスを設定してください。設定と履歴は `%LOCALAPPDATA%\PDF2Markdown\state.sqlite3` に保存します。起動前に環境変数 `PDF2MARKDOWN_DATA_DIR` へ絶対パスを指定すると配置を変更できます。環境設定用 `.runtime` とOCRモデルの配置は今回変更していません。
...
- `storage/`: ユーザー別保存先とSQLiteによる設定・変換結果の保存
...
個人用のローカルアプリです。入力PDFはメモリー内で処理し、変換設定・結果・エラーはユーザー別のSQLiteへ保存します。変換結果は1件処理するごとに保存し、再起動後に履歴から復元できます。途中のPDFを自動で再変換する機能や、環境インストールの復旧はありません。モデルはディスクキャッシュに保存されます。複数PDFは逐次変換し、同じ変換器への呼び出しはロックで直列化します。複数ユーザー向けのリソース制御や認証、ジョブキュー、実行中PDFのキャンセルは未実装です。ネットワークへ公開する用途は追加実装が必要です。

設定・履歴を含む今回の開発区切りと手動確認手順は [試用準備の進捗](docs/TRIAL_MILESTONE.md) を参照してください。
