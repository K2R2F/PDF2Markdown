...
def render_settings() -> Settings:
    with st.sidebar:
        ...
        ocr_mode = st.radio('OCRの実行方法', ['auto', 'force', 'off'],
                            format_func=lambda x: {'auto': '必要な領域にOCR（標準）', 'force': '全ページにOCR', 'off': 'OCRなし（テキストPDF）'}[x])
        tables = st.toggle('表の構造を復元', value=True)
        images = st.toggle('画像をMarkdownに埋め込む', value=False,
                           help='Base64で画像を埋め込みます。ファイルサイズが増えます。ビューアによっては画像表示に非対応です。')
        max_pages = st.number_input('1ファイルのページ上限', min_value=1, max_value=1000, value=200,
                                    help='超過したPDFは失敗として報告します。途中で切り捨てません。')
        st.divider()
        st.markdown('### 保存先')
        st.text_input('保存先フォルダー', value=str(DEFAULT_SAVE_DIR), key='save_directory')
    ...
    return Settings(engine, language, ocr_mode, tables, images, int(max_pages))
