...
2026-09-16追記: 以下は初回点検時の記録です。バージョン不一致のready判定、直接の子プロセスの異常時処理、通信切断の検出などを修正しました。現在の確認結果と残課題は [敵対的検証報告](ADVERSARIAL_REVIEW_20260916.md) を参照してください。配布要件全体はまだ未達です。
